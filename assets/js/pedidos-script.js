...
function updateCart() {
    ...
    if(cart.length > 0) {
        ...
        cart.map((itemCart, index)=>{
            modelItem = modelsJson.find((itemBD)=>itemBD.id == itemCart.id);
        ...
        localStorage.setItem('desconto', desconto.toFixed(2))
        localStorage.setItem('total', total.toFixed(2))
        localStorage.setItem('img', modelItem.img)
        localStorage.setItem('nome', modelItem.name)
